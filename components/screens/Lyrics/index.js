...
import { View, Text, Animated, Button, TouchableOpacity, TextInput, StyleSheet, ActivityIndicator, Alert, ScrollView, FlatList, Image, BackgroundImage, Dimensions, ImageBackground } from 'react-native'
...
export default function Lyrics({ songName, songCoverArt, songArtist, showNothing, setshowNothing, setshowLyrics }) {
    const { width, height } = Dimensions.get('window')
    const { allSongs_context_ref, currentIndex_context_ref, nowPlayingLyrics_context, setnowPlayingLyrics_context } = useContext(MainContext)
...
useEffect(() => {
        ...
        listener2.current = EventRegister.addEventListener('songposition', (pos) => {
            if (pos) {
                  ...
                  >
                    {
                      ...
                      >
                        ...
                        {nowPlayingLyrics_context.map((item, i) => {
                          return (
                            <View
                             key={i.toString()}
