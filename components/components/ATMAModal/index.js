...
// Callbacks
onDrag, onOpen, onOpened, onClose, onClosed, onBackButtonPress, onPositionChange, onOverlayPress, onLayout, }, ref) => {
  ...
  //////////////my added methods  
    const modalHeight_local = endHeight - alwaysOpen;
        ...
        if (nativeEvent.oldState === react_native_gesture_handler_1.State.ACTIVE) {
            ...
            react_native_1.Animated.spring(translateY, {
                ...
                useNativeDriver: USE_NATIVE_DRIVER,
                 ...
                 position_ref.current = modalPositionValue;
            });
    ...
    const position_ref = React.useRef('initial');
       React.useEffect(() => {
    ...
    const handleGestureEvent = react_native_1.Animated.event([{ nativeEvent: { translationY: dragY } }], {
        ...
        listener: ({ nativeEvent: { translationY } }) => {
            var _a;
      if(translationY<=0) {
        if(position_ref.current == 'top') return
            onDrag(translationY + modalHeight_local  ); 
        ...
        if (panGestureAnimatedValue) {
          const offset =
            (_a =
              alwaysOpen !== null && alwaysOpen !== void 0
                ? alwaysOpen
                : snapPoint) !== null && _a !== void 0
              ? _a
              : 0;
